...
async def crop_submit(token: str):
    ...
    if session.is_video:
        result_path = await ffmpeg.crop_and_resize(
            session.file_path, output_path, x, y, w, h, resize_w, resize_h
        )
    else:
        result_path = await pillow_conv.crop_and_resize(
            session.file_path, output_path, x, y, w, h, resize_w, resize_h
        )
...
async def dashboard(dash_token: str):
    with get_db() as conn:
        user_row = conn.execute(
            "SELECT user_id FROM users WHERE dashboard_token = ?", (dash_token,)
        ).fetchone()
        if user_row is None:
            abort(404)
...
async def delete_link(dash_token: str, token: str):
    with get_db() as conn:
        ...
        if user_row is None:
            abort(404)
        conn.execute(
            "DELETE FROM short_links WHERE token = ? AND user_id = ?",
            (token, user_row["user_id"]),
        )
...
async def delete_file(dash_token: str, token: str):
    with get_db() as conn:
        ...
        if user_row is None:
            abort(404)
        row = conn.execute(
            "SELECT path FROM files WHERE token = ? AND user_id = ?",
            (token, user_row["user_id"]),
        ).fetchone()
        if row:
            Path(
                row["path"]
            ).parent.parent  # careful: don't rmtree beyond the token dir if you reuse layout
            import shutil

            shutil.rmtree(Path(row["path"]).parent, ignore_errors=True)
        conn.execute(
            "DELETE FROM files WHERE token = ? AND user_id = ?",
            (token, user_row["user_id"]),
        )
    return jsonify({"status": "ok"})
...
async def upload_submit(token: str):
    ...
    try:
        if session.kind == "convert":
            ...
            crop_session = create_crop_session(
                local_path, session.user_id, session.chat_id, is_video
            )
        ...
        await app.task_manager.bot.send_message(
            session.chat_id, escape_md(f"Job failed: {str(e)}")
        )
